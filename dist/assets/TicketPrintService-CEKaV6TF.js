...
`,`VA`].join(``)}var w=`POS-58`,T=(e,t,n)=>`<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="utf-8" />
  <title>${t}</title>
...
</html>`;function E(e){return new Promise((t,n)=>{let r=window.open(``,`_blank`,`width=420,height=720`);if(!r){n(Error(`Permite ventanas emergentes para imprimir el ticket.`));return}r.document.open(),r.document.write(e),r.document.close(),r.addEventListener(`afterprint`,()=>{r.close(),t()}),setTimeout(t,12e4)})}function D(e,t=`Ticket`,n=w){let r=e.match(/<body[^>]*>([\s\S]*)<\/body>/i)?.[1]??e;return E(T(`${e.match(/<head[^>]*>([\s\S]*)<\/head>/i)?.[1]??``}${r}`,t,n))}function O(e){return new Promise((t,n)=>{try{let r=e.output(`blob`),i=URL.createObjectURL(r),a=window.open(i,`_blank`);if(!a){URL.revokeObjectURL(i),n(Error(`Permite ventanas emergentes para imprimir el ticket.`));return}a.addEventListener(`load`,()=>{a.focus(),a.print()}),a.addEventListener(`afterprint`,()=>{URL.revokeObjectURL(i),a.close(),t()}),setTimeout(()=>{URL.revokeObjectURL(i),t()},12e4)}catch(e){n(e)}})}function k(e,t=w){return`Se enviara a ${t} (impresora predeterminada en Windows). Papel 58 mm.`}var A=[9600,115200,19200,38400];function j(){return navigator.serial??null}function M(){return!!j()}async function N(e){let t=null;for(let n of A)try{await e.open({baudRate:n});return}catch(n){t=n;try{await e.close()}catch{}}throw t instanceof Error?t:Error(`No se pudo abrir la ticketera. Revisa cable USB y driver.`)}function P(e){let t=new Uint8Array(e.length);for(let n=0;n<e.length;n+=1)t[n]=e.charCodeAt(n)&255;return t}async function F(){let e=j();if(!e)throw Error(`Usa Chrome o Edge para conectar la PR-100 por USB/Serial.`);await e.requestPort()}async function I(e){let t=j();if(!t)throw Error(`Usa Chrome o Edge para imprimir directo en la PR-100.`);let n=(await t.getPorts())[0]??await t.requestPort();await N(n);try{let t=n.writable?.getWriter();if(!t)throw Error(`La ticketera no acepto datos por USB/Serial.`);await t.write(P(e)),t.releaseLock(),await new Promise(e=>setTimeout(e,400))}finally{try{await n.close()}catch{}}}function L(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function R(e){return`$${e.toFixed(2)}`}function z(e,n){let r=t(n),i=String(e.orderId||``).slice(-8).toUpperCase(),a=e.paymentLabel||e.method||`Efectivo`,o=Number(e.total||0),s=Math.round(o/1.13*100)/100,c=Math.round((o-s)*100)/100,l=n===`58mm`?`10px`:`11px`,u=e.items.map(e=>{let t=Number(e.quantity||0),n=Number(e.lineTotal||0),r=t>0?n/t:0;return`
        <tr>
          <td class="qty">${t}</td>
          <td class="name">${L(e.name||`Producto`)}</td>
          <td class="num">${R(r)}</td>
          <td class="num">${R(n)}</td>
        </tr>`}).join(``),d=e.method===`efectivo`?(()=>{let t=Number(e.cashReceived??o),n=Number(e.changeAmount??t-o),r=n<0?`-${R(Math.abs(n))}`:R(n);return`
            <div class="row"><span>Efectivo</span><span>${R(t)}</span></div>
            <div class="row"><span>Cambio</span><span>${r}</span></div>`})():`<div class="row"><span>Pagado</span><span>${R(o)}</span></div>`;return`<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="utf-8" />
  <title>Ticket ${L(i)}</title>
  <style>
...
<body>
  <div class="center">
    <p class="store">${L(e.storeName)}</p>
    ${e.subtitle?`<p class="subtitle">${L(e.subtitle)}</p>`:``}
  </div>
  <div class="meta">
    <div>Doc N°: <strong>${L(i)}</strong></div>
    <div>Fecha: ${L(e.date)}</div>
    <div>Caja: 1</div>
  ...
  <div class="totals">
    <div class="row"><span>Subtotal (sin IVA)</span><span>${R(s)}</span></div>
    <div class="row"><span>IVA 13%</span><span>${R(c)}</span></div>
    <div class="row total"><span>TOTAL</span><span>${R(o)}</span></div>
    <div class="row"><span>Método de pago</span><span>${L(a)}</span></div>
    ${d}
  </div>
...
</html>`}function B(e){let n=t(e);return`<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="utf-8" />
...
</html>`}function V(e){return e?.trim()||`POS-58`}async function H(e){if(!M())return!1;try{return await I(e),!0}catch{return!1}}async function U(e,t){let n=V(t.printerName),r=S(e,t.paperSize),i=z(e,t.paperSize);try{return await D(i,`Ticket de venta`,n),{method:`html`,printer:n}}catch(e){if(await H(r))return{method:`serial`,printer:n};if(t.pdfFallback)try{return await O(t.pdfFallback),{method:`pdf`,printer:n}}catch{}throw e instanceof Error?e:Error(`No se pudo imprimir en ${n}. Revisa la cola de impresion.`)}}async function W(e){let t=V(e.printerName);try{return await D(B(e.paperSize),`Prueba POS-58`,t),{method:`html`,printer:t}}catch{if(await H(C()))return{method:`serial`,printer:t};throw Error(`No se pudo imprimir en ${t}.`)}}async function G(){await F()}export{k as a,M as i,U as n,n as o,W as r,G as t};