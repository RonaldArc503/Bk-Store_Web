...
export default function POS() {
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const product = products.find((p) => p.codigo === barcodeScan)
    if (product) {
      addToCart(product)
      setBarcodeScan('')
    } else if (barcodeScan.trim() !== '') {
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <main className="flex-1 p-6">
        ...
        <div className="space-y-4">
          <form onSubmit={handleBarcodeSubmit} className="flex gap-3 max-w-3xl">
            ...
            <button className="px-6 py-3 bg-[#8CC63F] text-white rounded-xl">Agregar</button>
          </form>
